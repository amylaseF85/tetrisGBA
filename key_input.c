...
main(){
    ...
    */
    for (;;) {
        ...
        else                 draw_char(fb + 10, COLOR_BLACK, 'L');
        if (! (key & KEY_R)) draw_char(fb + 20, COLOR_WHITE, 'R' );
        else                 draw_char(fb + 20, COLOR_BLACK, 'R');
        ...
        else                     draw_char(fb + 70, COLOR_BLACK, 'S');
        if (! (key & KEY_SELECT)) draw_char(fb + 80, COLOR_WHITE, 's' );
        else                      draw_char(fb + 80, COLOR_BLACK, 's');
        if (! (key & KEY_B)) draw_char(fb + 90, COLOR_WHITE, 'B' );
        else                 draw_char(fb + 90, COLOR_BLACK, 'B');
        if (! (key & KEY_A)) draw_char(fb + 100, COLOR_WHITE, 'A');
        else                 draw_char(fb + 100, COLOR_BLACK, 'A');
    }
}
